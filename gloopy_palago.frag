	...
	vec3 hex_fract)
{
    ...
    vec2 square_fract = convert_hex_fract_to_square_fract(hex_fract);
    
    float dist_from_endpoint = sqrt(pow((1.0 - square_fract.x), 2.0) + pow(square_fract.y, 2.0));
    
    return (dist_from_endpoint - 0.5);
}
	...
	vec3 hex_fract)
{
    ...
    float half_shape = min(
        half_hex_fract.x, 
        get_palago_endcap_shape(half_hex_fract));
...
float get_opened_palago_shape(
	vec3 hex_fract,
	float color_polarity_sign)
{
    return (color_polarity_sign * get_palago_endcap_shape(wrap_hex_fract_into_x_tridant(color_polarity_sign * hex_fract)));
}
...
void main()
{
    ...
    const float min_rotation_speed = 0.008;
    const float max_rotation_speed = 0.012;
    ...
    float mixed_palago_shape = mix(
        ...
        mix(
            get_opened_palago_shape(hex_fract, sign(tile_rotation_velocity)),
            next_palago_shape,
            smoothstep(0.95, 1.0, rotation_fract)),
        smoothstep(0.9, 0.95, rotation_fract));
    ...
    //color = fract((get_first_tridant_mirrored_hex_fract(hex_fract).xyz * vec3(0.5)) + vec3(0.5));
